...
class ConfigGenerator:
  ...
  def get_metrics(self):
    ...
    # print("{0}".format(metrics))
    return_value = []
    metric_alarms = ['LogicalDisk % Free Space', 'Memory % Committed Bytes In Use', 'CPUCreditBalance', 'CPUUtilization', 'StatusCheckFailed', 'StatusCheckFailed_Instance', 'StatusCheckFailed_System']
  ...
  def get_alarm_disk_used_percent(self, metric, critical_level='warning', critical_threshold='80'):
    ...
    return_value['alarm_description'] = "'[{0}] high disk usage on {1}:{2} on path {3}'".format(critical_level.upper(), self.dimension_key, self.dimension_value, drive_instance)
    return_value['alarm_name'] = "'[{0}] high disk usage on {1}:{2} on path {3}'".format(critical_level.upper(), self.dimension_key, self.dimension_value, drive_instance)
    return_value['threshold'] = critical_threshold  # Default value is '80'
  ...
  def get_alarm_mem_used_percent(self, metric, critical_level='warning', critical_threshold='95'):
    ...
    return_value['threshold'] = critical_threshold  # Default value is '95'
    return_value['actions_enabled'] = True
    return_value['alarm_actions'] = [self.in_alarm]
    return_value['ok_actions'] = [self.ok_alarm]
    return_value['metric_name'] = metric['MetricName']
  ...
  def get_alarm_cpu_utilization(self, metric, critical_level='warning', critical_threshold='90'):
    return_value = {}
    return_value['comparison_operator'] = 'GreaterThanThreshold'
    ...
    return_value['threshold'] = critical_threshold  # Default value is '90'
    return_value['actions_enabled'] = True
    return_value['alarm_actions'] = [self.in_alarm]
    return_value['ok_actions'] = [self.ok_alarm]
    return_value['metric_name'] = metric['MetricName']
  ...
  def get_alarm_cpu_credit_balance(self, metric, critical_level='warning', critical_threshold='100'):
    return_value = {}
    ...
    return_value['threshold'] = critical_threshold  # Default value is '100'
    return_value['actions_enabled'] = True
    return_value['alarm_actions'] = [self.in_alarm]
    return_value['ok_actions'] = [self.ok_alarm]
    return_value['metric_name'] = metric['MetricName']
  ...
  def generate_yaml(self, metrics):
    ...
    for metric in metrics:
      ...
      metric_name = metric['MetricName']
      alarm_data = {}
      if metric_namespace == 'CWAgent' and metric_name == 'LogicalDisk % Free Space':
      ...
      elif metric_namespace == 'CWAgent' and metric_name == 'Memory % Committed Bytes In Use':
        ...
        critical_alarm_data = self.get_alarm_cpu_credit_balance(metric, 'critical', '50')
        critical_alarm_data['name'] = "{0}{1:03d}".format(prefix_name, ii)
        ii += 1
        parsed_data.append(critical_alarm_data)
